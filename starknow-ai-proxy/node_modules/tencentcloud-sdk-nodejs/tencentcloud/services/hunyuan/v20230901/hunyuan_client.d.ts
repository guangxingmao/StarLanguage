...
import { RunThreadRequest, SubmitHunyuanImageChatJobRequest, QueryHunyuanImageJobRequest, QueryHunyuanImageChatJobRequest, SetPayModeResponse, SubmitHunyuanImageJobResponse, GetThreadResponse, FilesDeletionsRequest, RunThreadResponse, ChatCompletionsResponse, GetThreadMessageListRequest, Submit3DSmartTopologyJobRequest, GetThreadMessageRequest, SubmitHunyuanImageChatJobResponse, ChatTranslationsRequest, GetThreadMessageResponse, GroupChatCompletionsRequest, ImageQuestionResponse, SetPayModeRequest, FilesDeletionsResponse, GetThreadMessageListResponse, CreateThreadRequest, Submit3DSmartTopologyJobResponse, Describe3DSmartTopologyJobRequest, ChatCompletionsRequest, FilesUploadsRequest, GetEmbeddingRequest, GroupChatCompletionsResponse, SubmitHunyuanImageJobRequest, GetTokenCountResponse, TextToImageLiteResponse, TextToImageLiteRequest, QueryHunyuanImageJobResponse, Describe3DSmartTopologyJobResponse, FilesListRequest, GetTokenCountRequest, ActivateServiceRequest, GetThreadRequest, ChatTranslationsResponse, ImageQuestionRequest, QueryHunyuanImageChatJobResponse, GetEmbeddingResponse, CreateThreadResponse, ActivateServiceResponse, FilesUploadsResponse, FilesListResponse } from "./hunyuan_models";
/**
 * hunyuan client
 * @class
 */
...
默认提供1个并发，代表最多能同时处理1个已提交的任务，上一个任务处理完毕后，才能开始处理下一个任务。
     */
    Describe3DSmartTopologyJob(req: Describe3DSmartTopologyJobRequest, cb?: (error: string, rep: Describe3DSmartTopologyJobResponse) => void): Promise<Describe3DSmartTopologyJobResponse>;
    /**
     * 腾讯混元大模型是由腾讯研发的大语言模型，具备强大的中文创作能力，复杂语境下的逻辑推理能力，以及可靠的任务执行能力。本接口支持流式或非流式调用，当使用流式调用时为 SSE 协议。
...
默认提供1个并发，代表最多能同时处理1个已提交的任务，上一个任务处理完毕后，才能开始处理下一个任务。
     */
    Submit3DSmartTopologyJob(req: Submit3DSmartTopologyJobRequest, cb?: (error: string, rep: Submit3DSmartTopologyJobResponse) => void): Promise<Submit3DSmartTopologyJobResponse>;
    /**
     * 混元生图（多轮对话）接口基于混元大模型，将根据输入的文本描述生成图像，支持通过多轮对话的方式不断调整图像内容。分为提交任务和查询任务2个接口。
提交任务：输入文本和前置对话 ID 等，提交一个混元生图多轮对话异步任务，获得任务 ID。
查询任务：根据任务 ID 查询任务的处理状态、处理结果，任务处理完成后可获得在上一轮对话基础上继续生成的图像结果。
